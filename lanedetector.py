...
def laneDetect(img, steer_prev):
    ...
    mask_yellow = cv2.inRange(hsv, (20, 0, 80),(100, 255, 255))
    mask_white = cv2.inRange(hsv, (0, 0, 80),(255, 20, 255))
    mask = cv2.bitwise_or(mask_yellow, mask_white)
    img2 = cv2.bitwise_and(img, img, mask=mask)

    stencil = np.zeros_like(img[:,:,0])
    polygon = np.array([[0, 480], [0,300], [250,250], [390,250], [640,300], [640, 480]])
    ...
    # Detect lines
    lines = cv2.HoughLinesP(canny, 1, np.pi/180, 30, maxLineGap=500)
    ...
    if lines is None:
        return img_proc, img3, steer_prev
    ...
    if x1_left != 0 and x2_left != 0 and x1_right != 0 and x2_right != 0:
        ...
        #Átlag        
        avg_dist = (dist_left_prev + dist_right_prev)/2
        if abs(avg_dist) > 100:
            x_dest += int(avg_dist/4)
        ...
        #cv2.line(img_proc, (int(ox1_left), int(oy1_left)), (int(ox2_left), int(oy2_left)), (255, 0, 0), 3)
        ...
        #cv2.line(img_proc, (int(ox1_right), int(oy1_right)), (int(ox2_right), int(oy2_right)), (255, 0, 0), 3)
    
        
    elif x1_left != 0 and x2_left != 0:
        m_left = (y1_left - y2_left) / (x1_left - x2_left)
        b_left = y1_left - m_left * x1_left
        
        x_dest = (256 - b_left) / m_left
        #Modification
        if dist_left_prev > -300:
            x_dest += 15
    ...
    elif x1_right != 0 and x2_right != 0:
        m_right = (y1_right - y2_right) / (x1_right - x2_right)
        b_right = y1_right - m_right * x1_right
        
        x_dest = (256 - b_right) / m_right
        #Modification
        if dist_right_prev < 300:
            x_dest -= 15
